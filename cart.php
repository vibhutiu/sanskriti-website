...
<?php
    ...
    $cart_size=sizeof($row);
    
    if($cart_size==0)
    {
        echo "<h1 align='center'>No Items In The Cart</h1>";
    }
    
    while($i<$cart_size)
    {
        $pid=$row[$i]["pro_id"];
        $cat=$row[$i]["pro_category"];
        $s=$row[$i]["size"];
        ...
        if($cat=="clothing")
        {
            if($s=="S")
            {
                $stock=$row1["small_stock"];
            }
            else if($s=="M")
            {
                $stock=$row1["medium_stock"];
            }
            else if($s=="L")
            {
                $stock=$row1["large_stock"];
            }
            else if($s=="XL")
            {
                $stock=$row1["xlarge_stock"];
            }
            else
            {
               $stock=1; 
        ...
        $m1='<div class="col-lg-2"><img src="data:image/jpeg;base64,'.base64_encode($row1['image']). ' " class="image" /></div>';
        
        $m2='<div class="col-lg-4" style="position:relative; top:60px"><b>'.$row1['name'].'</b><br />'.$csize.'</div><div class="col-lg-1"><div class="form-group">
        
        <input class="price" class="form-control" type="number" style="float:left;width:50px; border:none; background-color:#FFFFFF; position:relative; top:60px; right:40px"value="'.$row1['price'].'" onchange="sum1()" disabled="disabled"></div></div>';
        
        $m3='<div class="col-lg-1"><div class="form-group">
        
        <input class="qnt form-control" class="form-control" type="number" min="1" max="'.$stock.'" style="float:left;  width:60px; position:relative; top:55px; right:15px;" value="'.$row[$i]['quantity'].'" onchange="sum1()" name="q[]"></div></div>';
        
        $m4='<div class="col-lg-2 tot2" style="position:relative; top:60px" class=""><b>Total:</b> <span class="tot"></span></div>';
        
        $m5='<form action="remove_from_cart.php" method="post"><div class="col-lg-2	">
        <input type="hidden" value="'.$row[$i]['id'].'" name="cart_id" />
        <button type="submit" class="btn"  style=" position:relative;float:left; top:55px; ">Remove</button></div></form></div></div>';
...
<!--<h4> Total:<span id="fin"></span></h4>-->
<form action="store_qnt.php" method="post" id="form1">
<?php
    
    if($cart_size)
    {
        $but='<button type="submit" class="btn" id="buy" name="vibhu" form="form1">Buy Now</button>';
    }
    else
    {
        $but='<button type="submit" class="btn" id="buy" name="vibhu" form="form1" disabled="disabled">Buy Now</button>';
    }
