...
<button type="button" class="btn btn-default" data-toggle="collapse" data-target="#demo" style="background:#660033; width:200px; color:#FFFFFF">Places</button>
  ...
  <form method="post" action="cloth_select_cat.php">
  ...
  <br />
  <br />
  <button type="button" class="btn btn-default" data-toggle="collapse" data-target="#demo1" style="background:#660033; width:200px; color:#FFFFFF">Gender</button>
...
<?php
    ...
    $sql="SELECT * FROM clothing WHERE place".$_SESSION['place']." and category".$_SESSION['sex'];
    ...
    {			
        ...
        if ($result->num_rows > 0)
        {
            ...
            {
                echo "<div class='card' style='width:300px;height:420px; position:relative; top:15px; margin-left:23px; margin-right:23px;'><form action='clothing_preview.php' method='post'><input type='hidden' value='".$row['id']."' name='hide_id'/><input type='hidden' value='clothing' name='hide_cat'/><img src='data:image/jpeg;base64,".base64_encode($row['image'])."' class='image' /><p align='center' style='padding:0px;' ><button type='submit' class='btn btn-link'><b>" .$row["name"]. "</b></button> <br /><b>".$row["price"]."</b></p></form></div>";
            }
        }
        else
        {
